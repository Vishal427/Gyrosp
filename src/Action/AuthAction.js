import {authStatus,regStatus,otpStatus,logoutStatus} from "./Status";
// import axios from '../helper/Axios';
import axios from 'axios';
export const LoginNew=(userLogin)=>{ 
	console.log(userLogin)
	return async (dispatch)=>{ 
		dispatch({type:authStatus.LOGIN_REQUEST});
		const res = await axios.post("https://api.gyros.farm/api/AdminApi/LoginWithEmail",userLogin);
...
export const RegPage=(userReg)=>{
	...
	return async (dispatch)=>{
      ...
      const res = await axios.post("https://api.gyros.farm/api/AdminApi/Registration",userReg); 
      if(res.status===200){
      	if(res.data){
      		const {token,user} = res.data;
